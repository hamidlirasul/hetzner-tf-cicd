...
- **Provider not found**: run `terraform init -upgrade` and ensure `versions.tf` exists.
- **SSH key invalid**: your `ssh_public_key_path` must point to the **.pub** file that starts with `ssh-ed25519`.
- **Hetzner firewall error**: outbound rules must include `destination_ips`.
- **Docker Hub denied**: `APP_IMAGE` must be `yourdockerhubusername/reponame` (lowercase). Token must belong to that account.
- **Port not reachable**: confirm firewall allows `80`, container maps `-p 80:8000`, and the app listens on `0.0.0.0:8000`.

---

## 📌 Next Steps / Enhancements

- HTTPS with Caddy/Traefik (automatic TLS)
- Zero‑downtime deploy (blue/green or rolling)
- Prometheus + Grafana for metrics
- Centralized logs (Loki/ELK)
- Terraform remote backend + workspaces
- Secrets management (Vault or GitHub OIDC → cloud secrets)
